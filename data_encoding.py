...
def kmer_pairs(sequence):
    ...
    central_nucleotide = sequence[20]
...
def read_dataset(data_file):
    ...
    seq = df['seq']
    label = df['label']
    X = Kmer_Dataset(seq, label, KN_PairMatrix_values)
...
def save_features(features, filename, base_dir=r"/home/ys/SP/1_5/my_model/case_study"):
...
def process_full_pipeline(csv_path, kmer_params):
    ...
    save_features(kmer_features, "KN_features_test.npy")
    save_features(bert_embeddings.cpu().numpy(), "bert_embeddings_test.npy")
    save_features(one_hot_ncp_features, "one_hot_ncp_features_test.npy")
    save_features(labels, "labels.npy")

if __name__ == "__main__":
    input_csv = r"./dataset"
    KN_params = KN_PairMatrix_values
    process_full_pipeline(input_csv, KN_params)